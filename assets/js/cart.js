/* =========================================================
   Green Frog — Cart
   localStorage-backed pub-sub. Customizable lines key by
   id + options hash so two turtle lattes with different
   milks are separate lines.
   ========================================================= */

const STORAGE_KEY = 'greenfrog.cart.v1';
...
function renderDrawer(items) {
  ...
  for (const line of items) {
    ...
    const isCustomizable = window.FrogCustomize?.isCustomizable?.(line.id);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', e => {
    ...
    if (addBtn) {
      if (window.FrogCustomize?.isCustomizable?.(addBtn.dataset.id)) return;
      const item = {
    ...
    if (e.target.closest('[data-cart-checkout]')) {
      ...
      toast(`order placed — pickup at 820 Spring St ✦`);
      setTimeout(() => {
        Cart.clear();
        closeDrawer();
      }, 1500);
